...
// Distance funtion to obtain relative distances/co-ordinates for the 3D model
...
std::vector<cv::Point2d> get2dImagePoints1(full_object_detection& d) {
  ...
  imagePoints1.push_back(
      cv::Point2d(d.part(66).x(), d.part(66).y()));  // Stommion
  imagePoints1.push_back(
      cv::Point2d(d.part(36).x(), d.part(36).y()));  // Right Eye
  imagePoints1.push_back(
      cv::Point2d(d.part(45).x(), d.part(45).y()));  // Left Eye
  ...
  imagePoints1.push_back(
      cv::Point2d(d.part(28).x(), d.part(28).y()));  // Nose Point 1
  imagePoints1.push_back(
      cv::Point2d(d.part(29).x(), d.part(29).y()));  // Nose Point 2
  imagePoints1.push_back(
      cv::Point2d(d.part(17).x(), d.part(17).y()));  // Outer Eyebrow Tip Right
  imagePoints1.push_back(
      cv::Point2d(d.part(26).x(), d.part(26).y()));  // Outer Eyebrow Tip Left
  imagePoints1.push_back(
      cv::Point2d(d.part(21).x(), d.part(21).y()));  // Inner Eyebrow Tip Right
  imagePoints1.push_back(cv::Point2d(
      d.part(22).x(), d.part(22).y()));  // Inner Eyebrow Tip Left */
...
void method() {
  if ((depthCallbackBool && imgCallbackBool)) {
    ...
    for (unsigned long i = 0; i < faces.size(); ++i) {
      ...
      // Obtain depth values of chosen facial landmark points, these are the
      // applicates in the real world frame
      for (int i = 0; i < abscissae.size(); i++) {
        WorldFrameApplicates.push_back(
            depth_mat.at<float>(abscissae[i], ordinates[i]));
        cout << WorldFrameApplicates[i] << endl;
      }

      // Obtain the abscissae and ordinates of the real world co-ordinates in
      // the world frame

      for (int j = 0; j < abscissae.size(); j++) {
        WorldFrameAbscissae.push_back((WorldFrameApplicates[j] / cam_fx) *
                                      (abscissae[j] - cam_cx));
        WorldFrameOrdinates.push_back((WorldFrameApplicates[j] / cam_fx) *
                                      (ordinates[j] - cam_cy));
      }

      // store the abscissae, ordinates and applicates of the real world
      // co-ordinates in the world frame
      for (int k = 0; k < abscissae.size(); k++)
        RealWorld3D.push_back(cv::Point3d(WorldFrameAbscissae[k],
                                          WorldFrameOrdinates[k],
                                          WorldFrameApplicates[k]));
      ...
      cv::projectPoints(StomionPoint3D, rotationVector1, translationVector1,
      ...
      cv::projectPoints(modelPoints3D, rotationVector1, translationVector1,
                        cameraMatrix, distCoeffs, reprojectedPoints);
      // cv::projectPoints(modelPoints3DReal, rotationVector, translationVector,
      // cameraMatrix, distCoeffs, reprojectedPoints);
...
void DepthCallBack(const sensor_msgs::ImageConstPtr depth_img_ros) {
  ...
  depth_img_cv = cv_bridge::toCvShare(
      depth_img_ros,
      sensor_msgs::image_encodings::TYPE_16UC1);  // Get the ROS image to openCV
  depth_img_cv->image.convertTo(depth_mat, CV_32F,
                                0.001);  // Convert the uints to floats
